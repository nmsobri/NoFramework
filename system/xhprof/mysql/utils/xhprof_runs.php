...
class XHProfRuns_Default implements iXHProfRuns {
     ...
     *   c_url varchar(255) NULL,
     *   timestamp timestamp default CURRENT_TIMESTAMP NOT NULL ON UPDATE CURRENT_TIMESTAMP,
     *   `server name` varchar(64) NULL,
     *   perfdata mediumblob NULL,
     *   type tinyint NULL,
     *   cookie blob NULL,
     *   post blob NULL,
     *   get blob NULL,
     *   pmu int(11) UNSIGNED NULL,
     *   wt int(11) UNSIGNED NULL,
     *   cpu int(11) UNSIGNED NULL,
     *   server_id char(17) DEFAULT 't11' NOT NULL,
     *   aggregateCalls_include varchar(255) NULL
    ...
    public function save_run( $xhprof_data, $type, $run_id = null, $xhprof_details = null ) {
        ...
        We can't just clone the session data in start.php to be sneaky either, starting the session
        ...
        */
        ...
        $sql['url'] = urldecode( $this->db->escape( $url ) );
        $sql['c_url'] = urldecode( $this->db->escape( _urlSimilartor( $_SERVER['REQUEST_URI'] ) ) );
        ...
        # Remove data table crap
        $sql['url'] = preg_replace( "#(draw=.+?(?=&)|columns\[.+?\]\[.+?\](\[.+?\])?=.+?(?=&)|order\[.+?\]\[.+?\]=.+?(?=&)|start=.+?(?=&)|length=\d+?(?=&)|search\[.+?\]=.+?(?=&)|_=\d+?((?=&)|(?=$)))#", "", $sql['url'] );
        $sql['url'] = preg_replace( "#&((?=$)|(?=&))#", "", $sql['url'] );

        # Remove data table crap
        $sql['c_url'] = preg_replace( "#(draw=.+?(?=&)|columns\[.+?\]\[.+?\](\[.+?\])?=.+?(?=&)|order\[.+?\]\[.+?\]=.+?(?=&)|start=.+?(?=&)|length=\d+?(?=&)|search\[.+?\]=.+?(?=&)|_=\d+?((?=&)|(?=$)))#", "", $sql['c_url'] );
        $sql['c_url'] = preg_replace( "#&((?=$)|(?=&))#", "", $sql['c_url'] );
